...
AStar::CoordinateList AStar::Generator::findPath(Point start, Point end)
{
    ...
    while(i<size(path)-1)
    {
        ...
        while(j>i+1)
        {
            if(!detectCollision(path[i],path[j]))
            {
    ...
    if (size(path))
    {
        ...
        while(i<size(path)-1)
        {
           ...
           path.erase(path.begin()+(i+1),path.begin()+(MaxSkip-1));
           i++;
        }
...
*/
    /*
    ofstream OutFileB;
    OutFileB.precision(10);
    OutFileB.open("C:\\Users\\ccc0032\\Documents\\Load.obj", ios::out);
    OutFileB << "o Tree\n";
    for (int i=size(path)-1;i>=0; i--) {
        Point RealCoord = CoordToPoint(path[i]);
        OutFileB << "v " << RealCoord.x << " " << RealCoord.y << " " << RealCoord.z << "\n";
    }
    for (int i=size(path)-1;i>=1; i--) {
        OutFileB << "l " << i << " " << i+1 << "\n";
    }
...
void PathfindingComplexItem::loadObstacleFile(const QString& filename)
{
    ...
    {
        double Coords[3] = {};
        ifstream ReadTest(filename.toStdString().c_str(), ios::in | ios::binary);
        while (ReadTest) {
            ReadTest.read((char*)&Coords, sizeof(double) * 3);
            Cloud.push_back(Point(Coords[0], Coords[1], Coords[2]+74.92));
...
void PathfindingComplexItem::pathToWaypoint(QGeoCoordinate startLoc,QGeoCoordinate endLoc)
{
    Path.clear();
       ...
       LatLonToUTMXY(startLoc.latitude(), startLoc.longitude(), 16, X, Y);
       Point start = Point(X, Y, RelativeAlt ? getLocationAltitude(Point(X,Y,0)) + StartAltInput : StartAltInput );


       LatLonToUTMXY(endLoc.latitude(), endLoc.longitude(), 16, X, Y);
